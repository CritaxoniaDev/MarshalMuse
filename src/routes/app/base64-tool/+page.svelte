<script>
	...
	async function loadHistoryFromFirebase(userId) {
		try {
			const historyDocRef = doc(db, 'base64History', userId);
	...
	async function saveHistoryToFirebase() {
		...
		try {
			...
			const historyDocRef = doc(db, 'base64History', user.uid);

			// Limit history items to store in Firebase (text content can be large)
			const historyToSave = history.map((item) => {
				// For text items, truncate long inputs/outputs to save space
				if (item.type === 'text') {
					return {
						...item,
						input: item.input.length > 1000 ? item.input.substring(0, 1000) + '...' : item.input,
						output: item.output.length > 1000 ? item.output.substring(0, 1000) + '...' : item.output
					};
				}
				// For image items, we don't store the actual base64 data in history
				if (item.type === 'image') {
					return {
						...item,
						// Don't store the actual image data in Firebase
						output: item.mode === 'encode' ? 'Base64 Image Data (stored locally)' : item.output
					};
				}
				return item;
			});
	...
	function processImage() {
		...
		if (imageMode === 'encode' && imageInput) {
			...
			try {
				...
				imageError =
					'Error decoding Base64 image. Please ensure the input is valid Base64 image data.';
	...
	function addToHistory(type, mode, input, output) {
		...
		saveHistoryToFirebase(); // Save to Firebase after updating history
	}
	...
	function clearHistory() {
		history = [];
		clearHistoryFromFirebase(); // Clear from Firebase as well
	}

	// Function to get user stats
	function getUserStats() {
		if (!history.length) return { total: 0, text: 0, image: 0 };
	...
	onMount(() => {
		onAuthStateChanged(auth, (userData) => {
			if (userData) {
				user = userData;
				loadHistoryFromFirebase(userData.uid); // Load history from Firebase
			} else {
				goto('/');
			}
		});
	...
	<div class="max-w-[1600px] mx-auto">
		...
		<div class="grid grid-cols-1 lg:grid-cols-[1fr_350px] gap-4">
						...
						<Card.Root>
							...
							<Card.Content class="space-y-4">
								...
								<div class="flex justify-center">
									<Button.Root class="gap-2 px-6 py-2" on:click={processText} disabled={!textInput}>
										<FileCode size={18} />
									...
									>
										{#if imagePreview}
											<div class="mb-4 relative max-w-md mx-auto">
												...
												<Button.Root variant="outline" size="sm" class="mt-2" on:click={resetImage}>
													<RotateCcw size={14} class="mr-1" />
													Clear Image
												</Button.Root>
											</div>
			...
			<div class="space-y-4">
						...
						{:else}
							<div class="grid grid-cols-3 gap-4 text-center">
								...
								<div class="bg-green-50 p-3 rounded-lg">
									...
									<div class="text-xs text-gray-600">Text Conversions</div>
								</div>
								<div class="bg-purple-50 p-3 rounded-lg">
									<div class="text-2xl font-bold text-purple-600">{getUserStats().image}</div>
									<div class="text-xs text-gray-600">Image Conversions</div>
								</div>
							</div>
						{/if}
					</Card.Content>
						...
						<div class="space-y-4">
							<div class="flex gap-3">
								...
								<div>
									...
									<p class="text-sm text-gray-600">
										Base64 is a binary-to-text encoding scheme used to represent binary data in
										ASCII format.
									</p>
								</div>
							</div>
							...
							<div class="flex gap-3">
								...
								<div>
									...
									<p class="text-sm text-gray-600">
										Base64 encoding increases file size by approximately 33% compared to the
										original binary.
									</p>
								</div>
							</div>
				...
				{#if savingToFirebase}
					<div
						class="flex items-center justify-center gap-2 p-2 bg-blue-50 text-blue-700 rounded-md text-sm"
					>
						<div
